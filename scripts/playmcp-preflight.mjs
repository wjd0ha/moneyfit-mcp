...
assert(!result.programs.some((program) => program.title.includes("R&D 우수성과 50선")), "카페 컨설팅 요청에 R&D 수행기업 전용 공고가 포함됐습니다.");

const incompleteCalled = await post({
  jsonrpc: "2.0",
  id: 4,
  method: "tools/call",
  params: {
    ...
    arguments: {
      ...
      businessProfile: {
        ...
        businessType: "음식점/카페",
        keywords: ["서울", "카페"],
        missingFields: ["fundingPurpose"],
        summary: "서울 성수에서 카페를 운영하는 2년차 개인사업자"
      }
    }
  }
});
assert(incompleteCalled.isError !== true, "지원 목적이 생략된 프로필에서 도구 호출이 실패했습니다.");
const incompleteText = incompleteCalled.content?.find((item) => item.type === "text")?.text ?? "";
const incompleteResult = JSON.parse(incompleteText);
assert(incompleteResult.programs?.length > 0, "지원 목적이 없어도 검토 가능한 후보를 먼저 보여줘야 합니다.");
...
console.log("- purpose match: unrelated conditional notices excluded");
console.log("- incomplete profile: missing fundingPurpose accepted without tool failure");
